...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    if(this!=NULL && nodeIndex>=0 && nodeIndex<=(ll_len(this))){
    	...
    	if(nodeIndex==0){
    		aux->pNextNode=this->pFirstNode;
    		this->pFirstNode=aux;
    	}
    	else
    	{
    		for(int i=nodeIndex;i<=ll_len(this);i++)
    		{
    			getNode(this,i-1)->pNextNode=aux;
    			aux->pNextNode=getNode(this,i+1);
    		}
...
{
	int returnAux = -1;
	 if(this!=NULL && this->size<=(ll_len(this))){
